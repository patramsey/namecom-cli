...
The defects themselves are filed upstream and reported in full in this
directory:

| # | Defect | Upstream |
|---|---|---|
| 1 | `option.WithoutRetries()` is silently ignored at client scope | [#3](https://github.com/namedotcom/core-api-go/issues/3) · [report](core-api-go-withoutretries-ignored.md) |
| 2 | Retry backoff sleeps with a bare `time.Sleep`, ignoring the request context | [#4](https://github.com/namedotcom/core-api-go/issues/4) · [report](core-api-go-backoff-ignores-context.md) |
...
reports were verified against a local copy of v1.33.2 and are small. Both are
...
and could be skipped, at the cost of a call occasionally appearing to hang for
up to the SDK's 60s `maxRetryDelay`.

## Reproducing the measurements

The two reports each carry a self-contained `main.go` that reproduces the
